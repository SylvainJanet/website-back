...
public class MenuItemToDtoLevel1Converter extends AbstractConverter<List<MenuItem>, List<MenuItemDto>> {
	...
	protected List<MenuItemDto> convert(List<MenuItem> input) {
		...
		return input.stream().map(mi -> new MenuItemDto(mi.getId(), mi.getVersion(), conv.convert(mi.getSubItems()),
				mi.getDepth(), mi.getItemRank(), mi.getMenuId(),
